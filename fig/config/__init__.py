...
class FigConfig(configparser.SafeConfigParser):
    ...
    def validate(self):
        for section, var, envvar in self.__class__.ENV_DEFAULTS:
            try:
                ...
                raise Exception(
                    f"Please provide environment variable {envvar} or configuration option {section}.{var}"
                ) from err


        if int(self.get('events', 'severity_threshold')) not in range(5):
            raise Exception('Malformed configuration: expected events.severity_threshold to be in range 0-5')
        if int(self.get('events', 'older_than_days_threshold')) not in range(
            10000
        ):
    ...
    def validate_backends(self):
        if not self.backends.issubset(self.ALL_BACKENDS) or len(self.backends) < 1:
            raise Exception(
                f'Malformed configuration: expected main.backends to be subset of "{self.ALL_BACKENDS}" and contain at least one'
            )
